...
export async function approveMemberBanRequest(req, res, next) {
    ...
    try {
        ...
        const result = await db.tx(async t => {
            ...
            const existingMemberBanRequestEntity = await t.oneOrNone(`
                SELECT
                    *
                FROM
                    ms_member_ban
            ...
            if (existingMemberBanRequestEntity == null) {
                winstonLogger.info(`${baseMessage} Approve member ban approval request flow failed because request with id of ${banRequestId} is not found.`)
                response.message = "Failed update member ban approval request."
                return {
                    updatedMemberBanRequest: null,
            ...
            const existingMemberBanRequest = await singleObjectSnakeCaseToCamelCasePropsConverter(reqIdentifier, existingMemberBanRequestEntity)
            if (existingMemberBanRequest.updatedAt != null) {
                winstonLogger.info(`${baseMessage} Approve member ban approval request flow failed because request with id of ${banRequestId} is already updated.`)
                response.message = "Failed update member ban approval request."
                return {
                    updatedMemberBanRequest: null,
            ...
            const updateUserRecordBanStatusQuery = update({
                banStatus: banStatus,
                updatedAt: currentDate,
                id: existingMemberBanRequest.memberUserId
            }, TblUserUpdateBanStatusColumnSet)
            await t.none(updateUserRecordBanStatusQuery)

            return {
                updatedMemberBanRequest: await singleObjectSnakeCaseToCamelCasePropsConverter(reqIdentifier, updatedMemberBanRequestEntity),
                statusCode: UserDomainGeneralSuccessStatusCode
            }
        })
